...
TEST_F(RecursionTermTest, Similar)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    EXPECT_TRUE(lhsrt.similar(lhsrt));
    
    auto rhsrt = R4CTerm(t4cint, {{pbx, 1}, {wpy, 2},}, Fraction(1, 7));
    
    EXPECT_TRUE(lhsrt.similar(rhsrt));
    
    rhsrt = R4CTerm(t4cint, {{pbx, 4}, {wpy, 2},}, Fraction(3, 7));
    
    EXPECT_TRUE(lhsrt.similar(rhsrt));
    
    t4cint = T4CIntegral(bpair, kpair, operi, 1, {opddr, opddc});
    
    rhsrt = R4CTerm(t4cint, {{pbx, 1}, {wpy, 2},}, Fraction(3, 7));
    
    EXPECT_FALSE(lhsrt.similar(rhsrt));
    
    EXPECT_FALSE(rhsrt.similar(lhsrt));
    ...
    EXPECT_TRUE(rhsrt.similar(lhsrt));
}
...
TEST_F(RecursionTermTest, MapOfFactors)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    const auto mfacts = std::map<Factor, int>({{pbx, 1}, {wpy, 2},});
    
    EXPECT_EQ(t4crt.map_of_factors(), mfacts);
}
...
TEST_F(RecursionTermTest, ShiftOrder)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    EXPECT_EQ(t4crt.shift_order(-2), r4crt);
    
    r4cint = T4CIntegral(bpair, kpair, operi, 3, {opddr, opddc});
    
    r4crt = R4CTerm(r4cint, {{pbx, 1}, {wpy, 2},}, Fraction(1, 3));
    
    EXPECT_EQ(t4crt.shift_order(1), r4crt);
    
    EXPECT_FALSE(t4crt.shift_order(-3));
}
...
TEST_F(RecursionTermTest, Auxilary)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    EXPECT_TRUE(t4crt.auxilary(2));
    
    EXPECT_FALSE(t4crt.auxilary(0));
    
    EXPECT_FALSE(t4crt.auxilary(1));
    
    EXPECT_FALSE(t4crt.auxilary(3));
}
