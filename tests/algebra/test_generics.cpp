...
TEST_F(GenericsTest, BaseForString)
{
    std::string str("Axial");
    
    auto bstr = gen::base<std::string, std::string>(str);
    
    EXPECT_EQ(bstr, "Axial");
    
    str.clear();
    
    bstr = gen::base<std::string, std::string>(str);
    
    EXPECT_FALSE(bstr.has_value());
}

TEST_F(GenericsTest, BaseForRecursionGroup)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    auto bint = gen::base<R4Group, I4CIntegral>(t4group);
    
    EXPECT_EQ(bint, I4CIntegral(t4cint));
    
    const auto d4cdist = R4CDist(d4crt, {r4crtb,});
    
    t4group = R4Group({t4cdist, r4cdist, d4cdist});
    
    bint = gen::base<R4Group, I4CIntegral>(t4group);
