...
TEST_F(RecursionGroupTest, SignatureBase)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    const auto r4cint = T4CIntegral(bpair, kpair, operi, 1, {opddr, opddc});
    ...
    const auto t4cint0 = T4CIntegral(bpair, kpair, operi, 1, {opddr, opddc});
    
    const auto r4cint0 = T4CIntegral(bpair, kpair, operi, 0, {opddr, opddc});
    
    const auto tsign = t4group.signature();
    
    EXPECT_EQ(tsign.base<I4CIntegral>(), I4CIntegral(t4cint0)); 
}

TEST_F(RecursionGroupTest, Factors)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    EXPECT_EQ(t4group.factors(), std::set<Factor>({pbx, wpy}));
}

TEST_F(RecursionGroupTest, MapOfFactors)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    const auto mfacts = std::map<Factor, int>({{pbx, 3}, {wpy, 8},});

    EXPECT_EQ(t4group.map_of_factors(), mfacts);
}
