...
TEST_F(RecursionExpansionTest, Similar)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    const auto r4crtc = R4CTerm(t4cint, {{wpy, 3},}, Fraction(1, 3));
    
    EXPECT_TRUE(t4cdist.similar(t4cdist));

    EXPECT_TRUE(t4cdist.similar(R4CDist(t4crt)));
    
    EXPECT_TRUE(t4cdist.similar(R4CDist(t4crt, {r4crta, r4crtb, r4crtb})));
    
    EXPECT_TRUE(t4cdist.similar(R4CDist(t4crt, {r4crta})));
    
    EXPECT_TRUE(t4cdist.similar(R4CDist(t4crt, {r4crtc})));
    
    EXPECT_FALSE(t4cdist.similar(R4CDist(r4crtc, {t4crt})));
}
...
TEST_F(RecursionExpansionTest, MinOrder)
{
    const auto operi = OperatorComponent("1/|r-r'|");
...
TEST_F(RecursionExpansionTest, Factors)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    EXPECT_EQ(t4cdist.factors(), std::set<Factor>({pbx, wpy})); 
}

TEST_F(RecursionExpansionTest, MapOfFactors)
{
    const auto operi = OperatorComponent("1/|r-r'|");
    ...
    const auto mfacts = std::map<Factor, int>({{pbx, 2}, {wpy, 5},});
    
    EXPECT_EQ(t4cdist.map_of_factors(), mfacts);
}
