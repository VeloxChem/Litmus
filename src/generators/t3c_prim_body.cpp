...
void
T3CPrimFuncBodyDriver::write_func_body(      std::ofstream& fstream,
                                       const I3CIntegral&   integral) const
{
    ...
    if (integral[0] > 0)
    {
        lines.push_back({1, 0, 2, "// Set up R(WA) distances"});

        lines.push_back({1, 0, 2, "auto wa_x = factors.data(idx_wa);"});

        lines.push_back({1, 0, 2, "auto wa_y = factors.data(idx_wa + 1);"});

        lines.push_back({1, 0, 2, "auto wa_z = factors.data(idx_wa + 2);"});
    }
...
T3CPrimFuncBodyDriver::_get_buffers_str(const std::vector<R3CDist>& rec_dists,
                                        const I3CIntegral&          integral) const
{
    ...
    {
        auto label = "pbuffer.data(" + t3c::get_index_label(tint);
...
bool
T3CPrimFuncBodyDriver::_find_integral(const std::vector<R3CDist>& rec_dists,
...
std::vector<std::string>
T3CPrimFuncBodyDriver::_get_buffers_str(const I3CIntegral&        integral,
                                        ...
                                        const std::array<int, 2>& rec_range) const
{
    ...
    auto label = "pbuffer.data(" + t3c::get_index_label(integral);
...
std::string
T3CPrimFuncBodyDriver::_get_tensor_label(const I3CIntegral& integral) const
{
    std::string label;
...
std::string
T3CPrimFuncBodyDriver::_get_tensor_label(const T3CIntegral& integral) const
{
    std::string label;
...
void
T3CPrimFuncBodyDriver::_add_recursion_loop(      VCodeLines&         lines,
...
std::string
T3CPrimFuncBodyDriver::_get_pragma_str(const I3CIntegral&          integral,
...
void
T3CPrimFuncBodyDriver::_get_factor_lines(                VCodeLines& lines,
                                         const std::vector<R3CDist>& rec_distributions) const
{
    std::set<std::string> tlabels;
    ...
    if (std::find(tlabels.begin(), tlabels.end(), "fi_acd_0") !=  tlabels.end())
    {
        lines.push_back({2, 0, 2, "const double fi_acd_0 = 0.5 / (a_exp + c_exps[i] + d_exps[i]);"});
    }
    ...
    if (std::find(tlabels.begin(), tlabels.end(), "fz_be_0") !=  tlabels.end())
    {
        if (std::find(tlabels.begin(), tlabels.end(), "fi_acd_0") !=  tlabels.end())
        {
            lines.push_back({2, 0, 2, "const double fz_be_0 = 2.0 * (c_exps[i] + d_exps[i]) * fbe_0 * fi_acd_0;"});
        }
        else
        {
            lines.push_back({2, 0, 2, "const double fz_be_0 = (c_exps[i] + d_exps[i]) * fbe_0 / (a_exp + c_exps[i] + d_exps[i]);"});
        }
    }
    ...
    if (std::find(tlabels.begin(), tlabels.end(), "fzi_cd_0") !=  tlabels.end())
    {
        lines.push_back({2, 0, 2, "const double fzi_cd_0 = fi_cd_0 * a_exp / (a_exp + c_exps[i] + d_exps[i]) ;"});
    }
...
R3CDist
T3CPrimFuncBodyDriver::_get_vrr_recursion(const T3CIntegral& integral) const
{
    R3CDist rdist;
    
    if (integral.integrand().name() == "1/|r-r'|")
    {
        T3CVrrElectronRepulsionDriver eri_drv;

        if (integral[0].order() > 0)
        {
            rdist = eri_drv.apply_bra_vrr(R3CTerm(integral));
        }
        else
        {
            rdist = eri_drv.apply_ket_vrr(R3CTerm(integral));
        }
    }
...
std::string
T3CPrimFuncBodyDriver::_get_code_line(const R3CDist& rec_distribution) const
{
...
std::string
T3CPrimFuncBodyDriver::_get_rterm_code(const R3CTerm& rec_term,
...
std::string
T3CPrimFuncBodyDriver::_get_component_label(const T3CIntegral& integral) const
{
