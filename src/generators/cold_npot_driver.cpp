...
ColdNuclearPotentialDriver::ColdNuclearPotentialDriver()
{
...
bool
ColdNuclearPotentialDriver::is_nuclear_potential(const R2CTerm& rterm) const
{
...
ColdNuclearPotentialDriver::bra_vrr(const R2CTerm& rterm,
                                   const char     axis) const
{
    ...
    if (const auto tval = rterm.shift(axis, -1, 0))
    {
        ...
        if (const auto r2val = tval->shift_order(1))
        {
            ...
            x2val.add(Factor("AC", "rac", coord), Fraction(-1));
            ...
            x2val.add(Factor("BC", "rbc", coord), Fraction(-1));
        ...
        if (const auto r3val = tval->shift(axis, -1, 0))
        {
            ...
            if (const auto r4val = r3val->shift_order(1))
            {
                ...
                x4val.add(Factor("T", "t"), Fraction(-na, 2));
        ...
        if (const auto r5val = tval->shift(axis, -1, 1))
        {
            ...
            if (const auto r6val = r5val->shift_order(1))
            {
                ...
                x6val.add(Factor("T", "t"), Fraction(-nb, 2));
...
std::optional<R2CDist>
ColdNuclearPotentialDriver::ket_vrr(const R2CTerm& rterm,
                                   const char     axis) const
{
    if (!is_nuclear_potential(rterm)) return std::nullopt;
...
R2CDist
ColdNuclearPotentialDriver::apply_bra_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
R2CDist
ColdNuclearPotentialDriver::apply_ket_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
void
ColdNuclearPotentialDriver::apply_recursion(R2CDist& rdist) const
{
...
void
ColdNuclearPotentialDriver::apply_bra_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(0))
    {
...
void
ColdNuclearPotentialDriver::apply_ket_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(1))
    {
        R2CDist new_dist(rdist.root());
...
R2Group
ColdNuclearPotentialDriver::create_recursion(const VT2CIntegrals& vints) const
{
...
void
ColdNuclearPotentialDriver::apply_recursion(R2Group& rgroup) const
{
