...
#ifndef t2c_proj_prim_body_hpp
#define t2c_proj_prim_body_hpp
...
// Two-center projected ECP compute function body generators for CPU.
class T2CProjECPPrimFuncBodyDriver
{
    ...
    /// Computes common VRR recursion for integral component.
    ...
    /// Computes reduced VRR recursion for integral component.
    /// @param integral The base two center integral component.
    /// @return The recursion expansion of integral component.
    R2CDist _get_red_vrr_recursion(const T2CIntegral& integral) const;
    ...
    /// @param integral The base two center integral.
    /// @return The vector of buffer strings.
    std::vector<std::string> _get_red_buffers_str(const std::vector<R2CDist>& rec_dists,
                                                  const M2Integral&           integral) const;
    ...
    std::string _get_component_label(const T2CIntegral&        integral,
    ...
    /// @return The vector of buffer strings.
    std::vector<std::string> _get_buffers_str(const M2Integral&         integral,
    ...
    /// @param lines The code lines container to which loop start definition are added.
    /// @param rec_distributions The recursion distributions.
    void _get_red_factor_lines(                VCodeLines& lines,
                               const std::vector<R2CDist>& rec_distributions) const;
    ...
    /// @param components The vector of integral components.
    /// @param rec_range The recursion range [first, last) in integral components space.
    void _add_red_recursion_loop(      VCodeLines&         lines,
                                 const M2Integral&         integral,
                                 const VT2CIntegrals&      components,
                                 const std::array<int, 2>& rec_range) const;
    ...
    /// @param integral The base two center integral.
    /// @param rec_distributions The recursion distributions.
    std::string _get_pragma_str(const M2Integral&           integral,
                                const std::vector<R2CDist>& rec_distributions) const;
    ...
    /// Checks if coordinates of center A are required for integration.
    /// @param integral The base two center integral.
    bool _need_center_a(const I2CIntegral& integral) const;
    
    /// Checks if coordinates of center B are required for integration.
    /// @param integral The base two center integral.
    bool _need_center_b(const I2CIntegral& integral) const;
    
    /// Creates code string for recursion term.
    /// @param rec_term The recursion distribution.
    /// @return The string with code term.
    std::array<int, 3> _get_order(const R2CTerm& rec_term) const;
    
public:
    /// Creates a two-center compute function body generator.
    T2CProjECPPrimFuncBodyDriver() = default;
