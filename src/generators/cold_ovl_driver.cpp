...
ColdOverlapDriver::ColdOverlapDriver()
{
...
bool
ColdOverlapDriver::is_overlap(const R2CTerm& rterm) const
{
...
ColdOverlapDriver::bra_vrr(const R2CTerm& rterm,
                          const char     axis) const
{
    ...
    if (const auto tval = rterm.shift(axis, -1, 0))
    {
        ...
        if (const auto r2val = tval->shift(axis, -1, 0))
        {
            ...
            x2val.add(Factor("T", "t"), Fraction(na, 2));
...
std::optional<R2CDist>
ColdOverlapDriver::ket_vrr(const R2CTerm& rterm,
                           const char     axis) const
{
    if (!is_overlap(rterm)) return std::nullopt;
    ...
    if (const auto tval = rterm.shift(axis, -1, 1))
    {
        ...
        if (const auto r2val = tval->shift(axis, -1, 1))
        {
            ...
            x2val.add(Factor("T", "t"), Fraction(nb, 2));
...
R2CDist
ColdOverlapDriver::apply_bra_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
R2CDist
ColdOverlapDriver::apply_ket_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
void
ColdOverlapDriver::apply_recursion(R2CDist& rdist) const
{
...
void
ColdOverlapDriver::apply_bra_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(0))
    {
...
void
ColdOverlapDriver::apply_ket_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(1))
    {
        R2CDist new_dist(rdist.root());
...
R2Group
ColdOverlapDriver::create_recursion(const VT2CIntegrals& vints) const
{
...
void
ColdOverlapDriver::apply_recursion(R2Group& rgroup) const
{
