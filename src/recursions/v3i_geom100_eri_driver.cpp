...
bool
V3IGeom100ElectronRepulsionDriver::is_electron_repulsion(const I3CIntegral& integral) const
{
...
SI3CIntegrals
V3IGeom100ElectronRepulsionDriver::bra_hrr(const I3CIntegral& integral) const
{
    SI3CIntegrals tints;
    ...
    if (const auto tval = integral.shift(1, 0))
    {
        ...
        if (const auto r2val = integral.shift(-1, 0))
        {
            // second recursion term

            tints.insert(r2val->base());
        }
    }
...
SI3CIntegrals
V3IGeom100ElectronRepulsionDriver::apply_bra_hrr_recursion(const I3CIntegral& integral) const
{
    SI3CIntegrals tints;
