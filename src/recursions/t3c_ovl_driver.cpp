...
T3COverlapDriver::T3COverlapDriver()
{
...
bool
T3COverlapDriver::is_overlap(const R2CTerm& rterm) const
{
    ...
    if (rterm.integrand() != OperatorComponent("G(r)"))
    {
        return false;
    }
    else
...
T3COverlapDriver::bra_vrr(const R2CTerm& rterm,
                          const char     axis) const
{
    ...
    if (const auto tval = rterm.shift(axis, -1, 0))
    {
        ...
        x1val.add(Factor("GA", "ga", coord), Fraction(1));
...
std::optional<R2CDist>
T3COverlapDriver::ket_vrr(const R2CTerm& rterm,
                          const char     axis) const
{
    if (!is_overlap(rterm)) return std::nullopt;
    ...
    if (const auto tval = rterm.shift(axis, -1, 1))
    {
        ...
        x1val.add(Factor("GB", "gb", coord), Fraction(1));
        ...
        if (const auto r2val = tval->shift(axis, -1, 1))
        {
            ...
            x2val.add(Factor("1/geta", "gfe"), Fraction(nb));
...
R2CDist
T3COverlapDriver::apply_bra_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
R2CDist
T3COverlapDriver::apply_ket_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
void
T3COverlapDriver::apply_recursion(R2CDist& rdist) const
{
...
void
T3COverlapDriver::apply_bra_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(0))
    {
...
void
T3COverlapDriver::apply_ket_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(1))
    {
        R2CDist new_dist(rdist.root());
...
R2Group
T3COverlapDriver::create_recursion(const VT2CIntegrals& vints) const
{
...
void
T3COverlapDriver::apply_recursion(R2Group& rgroup) const
{
