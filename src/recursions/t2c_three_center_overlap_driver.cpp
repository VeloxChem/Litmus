...
T2CThreeCenterOverlapDriver::T2CThreeCenterOverlapDriver()
{
...
bool
T2CThreeCenterOverlapDriver::is_three_center_overlap(const R2CTerm& rterm) const
{
...
T2CThreeCenterOverlapDriver::bra_vrr(const R2CTerm& rterm,
                                     const char     axis) const
{
    if (!is_three_center_overlap(rterm)) return std::nullopt;
    ...
    if (const auto tval = rterm.shift(axis, -1, 0))
    {
        ...
        x1val.add(Factor("GA", "rga", coord), Fraction(1));
        ...
        if (const auto r2val = tval->shift(axis, -1, 0))
        {
            ...
            x2val.add(Factor("1/eta_abc", "fe_abc"), Fraction(na, 2));
        ...
        if (const auto r3val = tval->shift(axis, -1, 1))
        {
            ...
            x3val.add(Factor("1/eta_abc", "fe_abc"), Fraction(nb, 2));
        ...
        if (const auto r4val = tval->shift_operator(axis, -1))
        {
            ...
            x4val.add(Factor("1/eta_abc", "fe_abc"), Fraction(nc, 2));
...
std::optional<R2CDist>
T2CThreeCenterOverlapDriver::ket_vrr(const R2CTerm& rterm,
                                     const char     axis) const
{
    if (!is_three_center_overlap(rterm)) return std::nullopt;
    ...
    if (const auto tval = rterm.shift(axis, -1, 1))
    {
        ...
        x1val.add(Factor("GB", "rgb", coord), Fraction(1));
        ...
        if (const auto r2val = tval->shift(axis, -1, 1))
        {
            ...
            x2val.add(Factor("1/eta_abc", "fe_abc"), Fraction(nb, 2));
        ...
        if (const auto r3val = tval->shift_operator(axis, -1))
        {
            ...
            x3val.add(Factor("1/eta_abc", "fe_abc"), Fraction(nc, 2));
...
R2CDist
T2CThreeCenterOverlapDriver::apply_bra_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
R2CDist
T2CThreeCenterOverlapDriver::apply_ket_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
...
void
T2CThreeCenterOverlapDriver::apply_recursion(R2CDist& rdist) const
{
...
void
T2CThreeCenterOverlapDriver::apply_bra_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(0))
    {
        ...
        if (const auto nterms = rdist.terms(); nterms > 0)
        {
            ...
            {
                if (const auto rterm = rdist[i]; is_three_center_overlap(rterm))
                {
                    if (rterm.auxilary(0))
                    {
                        new_dist.add(rterm);
        ...
        {
            if (const auto rterm = rdist.root(); is_three_center_overlap(rterm))
            {
                rec_terms.push_back(rterm);
            }
        }
        ...
        while (!rec_terms.empty())
        {
            ...
            for (size_t i = 0; i < rec_terms.size(); i++)
            {
                ...
                if (const auto nterms = cdist.terms(); nterms > 0)
                {
                    ...
                    {
                        if (const auto rterm = cdist[j]; (rterm.auxilary(0) || (!is_three_center_overlap(rterm))))
                        {
                            new_dist.add(rterm);
                        }
                        else
...
void
T2CThreeCenterOverlapDriver::apply_ket_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(1))
    {
        R2CDist new_dist(rdist.root());
        ...
        while (!rec_terms.empty())
        {
            ...
            for (size_t i = 0; i < rec_terms.size(); i++)
            {
                ...
                if (const auto nterms = cdist.terms(); nterms > 0)
                {
                    for (size_t j = 0; j < nterms; j++)
                    {
                        if (const auto rterm = cdist[j]; (rterm.auxilary(1) || (!is_three_center_overlap(rterm))))
                        {
                            new_dist.add(rterm);
                        }
                        else
...
R2Group
T2CThreeCenterOverlapDriver::create_recursion(const VT2CIntegrals& vints) const
{
