...
T2CLocalECPDriver::full_bra_vrr(const R2CTerm& rterm,
                                const char     axis) const
{
    ...
    if (const auto tval = rterm.shift(axis, -1, 0))
    {
        ...
        if (const auto r3val = tval->shift(axis, -1, 1))
        {
            ...
            x3val.add(Factor("1/xi", "fxi"), Fraction(nb));
...
R2CDist
T2CLocalECPDriver::apply_full_bra_vrr(const R2CTerm& rterm) const
{
    R2CDist t2crt;
    ...
    {
        if (const auto trec = full_bra_vrr(rterm, axis))
        {
...
void
T2CLocalECPDriver::apply_full_recursion(R2CDist& rdist) const
{
    // vertical recursions on bra side
    
    apply_full_bra_vrr(rdist);
...
void
T2CLocalECPDriver::apply_full_bra_vrr(R2CDist& rdist) const
{
    if (!rdist.auxilary(0))
    {
        ...
        while (!rec_terms.empty())
        {
            ...
            {
                const auto cdist = apply_full_bra_vrr(rec_terms[i]);
...
R2Group
T2CLocalECPDriver::create_full_recursion(const VT2CIntegrals& vints) const
{
...
void
T2CLocalECPDriver::apply_full_recursion(R2Group& rgroup) const
{
    if (const auto nterms = rgroup.expansions(); nterms > 0)
    {
        R2Group mgroup;
