...
#ifndef t4c_center_driver_hpp
#define t4c_center_driver_hpp
...
/// Four center geometrical prefix operator driver class.
class T4CCenterDriver
{
...
public:
    ...
    bool is_auxilary(const R4CTerm& rterm,
    ...
    std::optional<R4CDist> bra_ket_vrr(const R4CTerm& rterm,
                                       const char     axis,
                                       const int      index) const;
    ...
    /// @param index The index of prefix operator.
    /// @return The recursion expansion of given recursion term.
    R4CDist apply_bra_ket_vrr(const R4CTerm& rterm,
                              const int      index) const;
    ...
    /// @param rdist The recursion expansion.
    /// @param index The index of prefix operator.
    void apply_bra_ket_vrr(      R4CDist& rdist,
                           const int      index) const;
